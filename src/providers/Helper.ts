...
export class Helper {
  ...
  assertUpgrade(): Observable<any> {
    ...
    let curNo=parseInt(this.globalData.appNo.replace('.','').replace('.',''));
    return Observable.create(observer => {
      this.nativeService.getPackageName().subscribe(packageName => {//获得app包名
        ...
        //let url = Utils.formatUrl(`${this.globalData.serverApkUrl}/app/${appName}/${appType}/latest/version`);
        let param = {
          //必传
          'action': 'judgeAppVersion',
          'appType':appType,//android,ios
          'appName': appName,
          'preVersion':curNo
        };
        
        //versionInfo:appName:upgradeDate:appType:isForcedUpdate:currentVersion

        //this.http.get(url).map((res: Response) => res.json()).subscribe(res => {//从后台查询app最新版本信息
        this.httpService.get('phoneMyInfo.do', param).map((res: Response) => res.json()).subscribe(res => {//从后台查询app最新版本信息
        let resultBase=res[0];
        let resultArray=res[1];
        if (resultBase.result=="false") {
          observer.next({update: false, msg: '获取更新信息出错'});
          return;
        }
        ...
        let resultInfo=resultArray[0];
        if (curNo == parseInt(resultInfo.currentVersion)) {//比较版本号
          observer.next({update: false, msg: '已经是最新版本'});
        } else {
          if (resultInfo.isForcedUpdate == 1) {//判断是否强制更新
